...
class RF:
    ...
    def forward(self, x, cond, mask):
        ...
        x = x * mask.unsqueeze(1).view(b, 1, x.shape[-2], x.shape[-1])
        vtheta = vtheta * mask.unsqueeze(1).view(b, 1, vtheta.shape[-2], vtheta.shape[-1])
        z1 = z1 * mask.unsqueeze(1).view(b, 1, z1.shape[-2], z1.shape[-1])
