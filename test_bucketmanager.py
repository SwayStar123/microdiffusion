...
class BucketManager:
    def __init__(self, bucket_file, valid_ids=None, max_size=(64,64), divisible=2, step_size=8, min_dim=32, base_res=(64,64), bsz=64, world_size=1, global_rank=0, max_ar_error=4, seed=42, dim_limit=128, debug=False):
    ...
    def find_bucket_for_image(self, image_id):
        """
        Finds the bucket ID and resolution for a given image ID.

        :param image_id: The ID of the image to find the bucket for.
        :return: A tuple of (bucket ID, resolution) if found, otherwise (None, None).
        """
        for bucket_id, image_ids in self.buckets.items():
            if image_id in image_ids:
                resolution = tuple(self.resolutions[bucket_id])
                return bucket_id, resolution
        return None, None

if __name__ == "__main__":
    bm = BucketManager(bucket_file="../../datasets/commoncatalog_cc_by_moondream_metadata/res_map.json", debug=False, bsz=4)

    print("Bucket for image ID 05922103:", bm.find_bucket_for_image(image_id="05922103"))